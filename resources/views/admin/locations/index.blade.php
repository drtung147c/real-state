...
@section('content')
@can(\App\Http\Resources\PermissionResource::LOCATION_CREATE)
                ...
                <tbody>
                    @foreach($locations as $key => $location)
                        <tr data-entry-id="{{ $location->id }}">
                            ...
                            <td>
                                @can(\App\Http\Resources\PermissionResource::LOCATION_SHOW)
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.locations.show', $location->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can(\App\Http\Resources\PermissionResource::LOCATION_EDIT)
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.locations.edit', $location->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can(\App\Http\Resources\PermissionResource::LOCATION_DELETE)
