...
<div class="card">
    ...
    <div class="card-body">
        <form id="edit-form" action="{{ route("admin.venues.update", [$venue->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="name">{{ trans('cruds.venue.fields.user_name') }}*</label>
                <input type="hidden" id="user_id" name="user_id" class="form-control" value="{{ $venue->users->id }}">
                <input type="text" id="user_name" class="form-control" value="{{ $venue->users->name }}" disabled>
            </div>
            ...
            <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                ...
                <input value="1" type="checkbox" id="status" name="status" {{ old('status', isset($venue) ? $venue->status : '') == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group {{ $errors->has('is_featured') ? 'has-error' : '' }}">
                ...
                <input value="1" type="checkbox" id="is_featured" name="is_featured" {{ old('features', isset($venue) ? $venue->is_featured : '') == 1 ? 'checked' : '' }}>
...
<script>
    ...
    success: function (file, response) {
      $('form#edit-form').find('input[name="main_photo"]').remove()
      $('form#edit-form').append('<input type="hidden" name="main_photo" value="' + response.name + '">')
    },
...
@if(isset($venue) && $venue->main_photo)
      ...
      $('form#edit-form').append('<input type="hidden" name="main_photo" value="' + file.file_name + '">')
      this.options.maxFiles = this.options.maxFiles - 1
@endif
    },
...
Dropzone.options.galleryDropzone = {
    ...
    success: function (file, response) {
      $('form#edit-form').append('<input type="hidden" name="gallery[]" value="' + response.name + '">')
    ...
    removedfile: function (file) {
      ...
      $('form#edit-form').find('input[name="gallery[]"][value="' + name + '"]').remove()
    },
    init: function () {
@if(isset($venue) && $venue->gallery)
      var files =
        {!! json_encode($venue->gallery) !!}
          ...
          file.previewElement.classList.add('dz-complete')
          $('form#edit-form').append('<input type="hidden" name="gallery[]" value="' + file.file_name + '">')
        }
@endif
    },
     error: function (file, response) {
