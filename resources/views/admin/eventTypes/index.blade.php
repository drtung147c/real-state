...
@section('content')
@can(\App\Http\Resources\PermissionResource::EVENT_TYPE_CREATE)
                ...
                <tbody>
                    @foreach($eventTypes as $key => $eventType)
                        <tr data-entry-id="{{ $eventType->id }}">
                            ...
                            <td>
                                @can(\App\Http\Resources\PermissionResource::EVENT_TYPE_SHOW)
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.event-types.show', $eventType->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can(\App\Http\Resources\PermissionResource::EVENT_TYPE_EDIT)
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.event-types.edit', $eventType->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can(\App\Http\Resources\PermissionResource::EVENT_TYPE_DELETE)
